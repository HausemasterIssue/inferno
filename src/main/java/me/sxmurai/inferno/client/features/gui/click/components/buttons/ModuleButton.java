...
import me.sxmurai.inferno.client.features.gui.click.components.other.Slider;
...
public class ModuleButton extends Button {
    ...
    private void init() {
        ...
        for (Value value : this.module.getSettings()) {
            if (value.getValue() instanceof Boolean) {
                this.components.add(new BooleanButton(value));
                continue;
            } else if (value.getValue() instanceof Enum) {
                this.components.add(new EnumButton(value));
                continue;
            }

            if (value.isNumberSetting()) {
                if (value.hasRestriction()) {
                    this.components.add(new Slider(value));
                } else {
                    // @todo
                }
